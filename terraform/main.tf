...
resource "azurerm_container_registry" "capstone" {
  ...
  # Capstone-safe default remains true to avoid breaking the existing demo workflow.
  # Production recommendation: set this to false and use GitHub OIDC + managed identity.
...
resource "azurerm_storage_account" "product_images" {
  ...
  allow_nested_items_to_be_public  = var.allow_public_product_images
  tags                             = local.common_tags
}
...
resource "azurerm_storage_container" "product_images" {
  ...
  container_access_type = var.product_images_container_access_type
}
